...
\begin{tad}{\tadNombre{Juego}}
  ...
  \tadAxioma{cantidadDeSanciones(j1, agJugador(j2, c, pGo))}{\IF j1 = j2 THEN 0 ELSE cantidadDeSanciones(j1) FI}
	...
	\tadAxioma{estaEnElRangoDeOtroPokemon(c, ps, pGo)}{\IF $\emptyset$?(ps) THEN false ELSE territorioOcupado(c,posicionPokemon($<$dameUno(ps), obtener(dameUno(ps), pokemones(pGo))$>$, pGo), mapa(pGo)) $\vee$ estaEnElRangoDeOtroPokemon(c,sinUno(ps),pGo) FI}
