...
\begin{tad}{\tadNombre{Juego}}
	...
	\tadIgualdadObservacional{pGo}{pGo'}{juego}
                         ...
                         ($\forall$ j: Jugador) def?(j,jugadores(j)) (posicionJugador(j,pGo) $\igobs$ posicionJugador(j,pGo') $\wedge$ \\ cantidadDeSanciones(j,pGo) $\igobs$ cantidadDeSanciones(j,pGo')) $\yluego$ \\
						 ($\forall$ j: Jugador) def?(j,jugadores(j)) pokemonesAtrapados(j,pGo) $\igobs$ pokemonesAtrapados(j,pGo') $\yluego$ \\
						 ($\forall$ p: Pokemon) def?($\Pi_1$(p),pokemones(pGo)) (posicionPokemon(p,pGo) $\igobs$ posicionPokemon(p,pGo')                          $\wedge$ \\ cuantoLlevaEsperando(p,pGo) $\igobs$ cuantoLlevaEsperando(p,pGo'))
                         }
...
%	\tadOperacion{posicionJugadores}{Coordenada/c, Juegao/pGo}{conj(Jugador)}{c $\in$ posiciones(mapa(pGo))}  
	...
	\tadOperacion{esSalvajeAux}{Nat/id, Conj(Nat)/js, Juego/pGo}{Bool}{def?(id,pokemones(pGo)) $\wedge$ js $\subseteq$ claves(jugadores(pGo))}        
    \tadOperacion{estaEnRangoDeAtrapar}{Jugador/j, Juego/pGo}{Bool}{def?(j, jugadores(pGo))} 
    \tadOperacion{estaEnRangoDeAtraparAux}{Jugador/j, Conj(Nat)/cp, Juego/pGo}{Bool}{def?(j, jugadores(pGo)) $\wedge$ cp $\subseteq$ claves(pokemones(pGo))} 
	\tadOperacion{pokemonDelRango}{Jugador/j, Juego/pGo}{Pokemon}{def?(j, jugadores(pGo)) $\yluego$ estaEnRangoDeAtrapar(j,pGo)}   
	\tadOperacion{pokemonDelRangoAux}{Jugador/j, Conj(Nat)/cp, Juego/pGo}{Pokemon}{def?(j, jugadores(pGo)) $\yluego$ estaEnRangoDeAtrapar(j,pGo) $\wedge$ cp $\subseteq$ claves(pokemones(pGo))}   
    \tadOperacion{jugadoresDelRango}{Coordenada/c, Juego/pGo}{Conj(Jugador)}{esPosicionValidaMapa(c, pGo)}
    \tadOperacion{jugadoresDelRangoAux}{Coordenada/c, Conj(Jugador)/js, Juego/pGo}{Conj(Jugador)}{esPosicionValidaMapa(c, pGo) $\wedge$ js $\subseteq$ claves(jugadores(pGo))}
    \tadOperacion{estaEnElRangoDeOtroPokemon}{Coordenada/c, Conj(Nat)/ps, Juego/pGo}{Bool}{esPosicionValidaMapa(c,pGo) $\wedge$ ps $\subseteq$ claves(pokemones(pGo))}
    ...
    \tadOperacion{cantidadPokemonesTipo}{Tipo/t, Conj(Nat)/ps, Juego/pGo}{Nat}{estaDefTipo(t,claves(pokemones(pGo))),pGo $\wedge$ ps $\subseteq$ claves(pokemones(pGo))}
...
%
	...
	\tadAxioma{estaEnElRangoDeOtroPokemon(c, ps, pGo)}{\IF $\emptyset$?(ps) THEN false ELSE territorioOcupado(c,dameUno(ps),pGo) $\vee$ estaEnElRangoDeOtroPokemon(c,sinUno(ps),pGo) FI}
  ...
  \tadAxioma{esSalvaje?(p, pGo)}{esSalvajeAux($\Pi_1$(p), claves(jugadores(pGo)), pGo} 
    ...
    \tadAxioma{estaEnRangoDeAtraparAux(j,cp,pGo)}{\IF $\emptyset$?(cp) THEN false ELSE estanEnElMismoRango(posicionJugador(j,pGo), posicionPokemon($\langle$dameUno(cp),obtener(dameUno(cp),pokemones(pGo))$\rangle$,pGo), mapa(pGo)) $\vee$ estaEnRangoDeAtraparAux(j,sinUno(cp),pGo) FI}
	\tadAxioma{jugadoresDelRango(c,pGo)}{jugadoresDelRangoAux(c,claves(jugadores(pGo)),pGo)}
	\tadAxioma{jugadoresDelRangoAux(c,js,pGo)}{\IF $\emptyset$?(js) THEN $\emptyset$ ELSE {\IF estaConectado?(dameUno(js),pGo) THEN {\IF estanEnElMismoRango(c,posicionJugador(dameUno(js)),mapa(pGo)) THEN Ag(dameUno(js),jugadoresDelRangoAux(c,sinUno(js),pGo)) ELSE jugadoresDelRangoAux(c,sinUno(js),pGo) FI} ELSE jugadoresDelRangoAux(c,sinUno(js),pGo) FI} FI}    

	\tadAxioma{pokemonDelRango(j,pGo)}{pokemonDelRangoAux(j,claves(pokemones(pGo)),pGo)}
	\tadAxioma{pokemonDelRangoAux(j,cp,pGo)}{\IF estanEnElMismoRango(posicionJugador(j,pGo), posicionPokemon($\langle$dameUno(cp),obtener(dameUno(cp),pokemones(pGo))$\rangle$,mapa(pGo))) THEN $\langle$dameUno(cp),obtener(dameUno(cp),pokemones(pGo))$\rangle$ ELSE pokemonDelRangoAux(j,sinUno(cp),pGo) FI}
