...
%	\tadOperacion{posicionJugadores}{Coordenada/c, Juegao/pGo}{conj(Jugador)}{c $\in$ posiciones(mapa(pGo))}  
	...
	\tadOperacion{esSalvajeAux}{Nat/id, Conj(Nat)/js}{Bool}{def?(id,pokemones(pGo)) $\wedge$ js $\subseteq$ claves(jugadores(pGo))}        
	...
	\tadOperacion{pokemonDelRango}{Jugador/j, Juego/pGo}{Pokemon}{def?(j, jugadores(pGo)) $\yluego$ estaEnRangoDeAtrapar(j,pGo)} %FaltanRestricciones si el j esta en rango de atrapar, te devuelve el pokemon en cuestión.    
    ...
    \tadOperacion{cantidadPokemonesTipo}{Tipo/t, Conj(Nat)/ids, Juego/pGo}{Nat}{estaDefTipo(t,claves(pokemones(pGo))),pGo}
    \tadOperacion{dividir}{Nat/n, Nat/d}{Nat}{$\neg$(d = 0?)}
    \tadOperacion{estaDefTipo}{Tipo/t, conj(nat)/claves, Juego/pGo}{bool}{}
...
%
  ...
  \tadAxioma{sacarPokemones(cp, pGo)}{{\IF vacio?(cp) THEN pokemones(pGo) ELSE sacarPokemonesAux(cp,pokemones(pGo)) FI}} 
  
    \tadAxioma{sacarPokemonesAux(cp, pokes)}{{\IF vacio?(cp) THEN pokes ELSE sacarPokemonesAux(sinUno(cp),borrar(dameUno(cp,pokes))) FI}}
    
      \tadAxioma{estaDefTipo(t, pokes, pGo)}{{{\IF vacio?(pokes) THEN false ELSE {\IF t =obtener(dameUno(pokes),pokemones(pGo)) THEN true ELSE estaDefTipo(t,sinUno(pokes),pGo) FI} FI}} }
  
  \tadAxioma{esSalvaje(p, pGo)}{esSalvajeAux($\Pi_1$(p), claves(jugadores(pGo)), pGo} 
  
    \tadAxioma{esSalvajeAux(id,js, pGo)}{{{\IF vacio?(js) THEN false ELSE {\IF definido?(id,pokemonesAtrapados(dameUno(js),pGo)) THEN true ELSE esSalvajeAux(id,sinUno(js),pGo) FI} FI}} }
